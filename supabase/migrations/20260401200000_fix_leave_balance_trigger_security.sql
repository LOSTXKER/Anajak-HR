-- Fix: update_leave_balance() trigger needs SECURITY DEFINER
-- so it can INSERT/UPDATE leave_balances when called by non-admin employees.
-- Without this, RLS blocks the trigger because only admins have write access to leave_balances.
