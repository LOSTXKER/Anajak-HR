 ...
 * Fetches all requests at once; pending filtering is handled client-side.
 */

import { useState, useCallback } from "react";
import { supabase } from "@/lib/supabase/client";
import { RequestItem, Employee, Holiday, OTSettings } from "@/lib/types/request";
...
export function useRequestsQuery(): UseRequestsQueryReturn {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      // Note: settings page saves "work_hours_per_day", query that key.
      ...
      if (settings.work_hours_per_day) {
        setHoursPerDay(parseFloat(settings.work_hours_per_day) || 8);
      }
    } catch (error) {
